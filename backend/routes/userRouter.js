...
        }
        const oldUser = await User.findById(req.params.id).select('+password').session(session);
        if (!oldUser) {
            return res.status(404).json({ success: false, message: 'User not found.' });
        }
        if (oldUser._id.toString() === req.user.id) {
            return res.status(403).json({ success: false, message: 'Admins cannot change their own role.' });
        }
        if (oldUser.role === newRole) {
            return res.status(200).json({ success: true, data: oldUser, message: `User role is already ${newRole}` });
        }
        
        const NewUserDiscriminator = User.discriminators[newRole];
        if (!NewUserDiscriminator) {
            throw new Error('Invalid new role for discriminator creation.');
        }
        
        const newUserData = { ...oldUser.toObject({ getters: true }), role: newRole, password: oldUser.password };
        delete newUserData.__v;

        const newUser = await NewUserDiscriminator.create([newUserData], { session });

        await User.findByIdAndDelete(oldUser._id, { session });
        
        await session.commitTransaction();
        
        res.status(200).json({ success: true, data: newUser[0], message: `User role updated to ${newRole}` });
    } catch (error) {
