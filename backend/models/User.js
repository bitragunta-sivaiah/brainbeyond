...
    // Field to validate the current session and enforce single-device login
    sessionToken: {
        type: String,
        select: false
    },
...
// Corrected Middleware for Role Change
baseUserSchema.pre('save', function (next) {
  // Check if the role is being changed
  if (this.isModified('role')) {
    // Manually mark the role path as `modified` but tell Mongoose not to validate it
    // This allows the router's atomic 'delete and create new' logic to proceed without
    // the document-level validator throwing an error.
    this.markModified('role');
    this.$ignoreChanges = ['role'];
    this.isNew = true; // Trick Mongoose into thinking it's a new doc, for the router's logic
  }
  next();
});
...
        throw new Error('Session token not set on user before signing JWT');
    }
    // Embed the session token into the JWT payload
    return jwt.sign(
...
    return this.oneTimeToken === hashedOtp;
};
...
        averageResolutionTime: Number, // in minutes or hours
        customerSatisfaction: Number, // e.g., on a scale of 1-5
