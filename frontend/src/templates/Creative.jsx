...
const IconText = ({ icon, text, link }) => (
  <div className="flex items-center gap-2 mb-2">
    {icon}
    {link ? <a href={link} target="_blank" rel="noopener noreferrer" className="text-xs text-gray-700 hover:underline">{text}</a> : <span className="text-xs text-gray-700">{text}</span>}
  </div>
);

const MainSectionHeader = ({ title }) => (
  <h2 className="text-sm font-bold uppercase text-[#1E40AF] tracking-wider mb-2 mt-4 first:mt-0">{title}</h2>
);
  ...
  return (
    <div className="flex bg-white text-[9pt] font-sans">
      ...
      <aside className="w-[30%] bg-gray-50 p-6">
        <h1 className="text-xl font-bold text-[#1E40AF]">{contact.firstName} {contact.lastName}</h1>
        <p className="text-sm text-gray-600 mt-1 mb-6">{contact.professionalTitle}</p>

        <SidebarSectionHeader title="Contact" />
        {contact.email && <IconText icon={<Mail size={12} className="text-gray-600"/>} text={contact.email} link={`mailto:${contact.email}`} />}
        {contact.phone && <IconText icon={<Phone size={12} className="text-gray-600"/>} text={contact.phone} />}
        {contact.website && <IconText icon={<Globe size={12} className="text-gray-600"/>} text="Website" link={contact.website} />}
          ...
          <>
            ...
            {skills.map(skill => (
              <div key={skill.category} className="mb-2">
                <p className="text-xs font-bold text-[#1E40AF]">{skill.category}</p>
