...
const SectionHeader = ({ title }) => (
  <div className="mb-2 mt-4">
    <h2 className="text-sm font-bold uppercase text-[#1E40AF] tracking-wider">
      {title}
    </h2>
    ...
    <div className="p-6 mx-auto bg-white text-gray-900 text-[9pt] font-sans leading-normal">
      ...
      <header className="text-center mb-4">
        ...
        <div className="flex justify-center items-center flex-wrap gap-x-2 text-xs text-gray-500 mt-2">
            {contact.email && <a href={`mailto:${contact.email}`} className="text-blue-700 hover:underline">{contact.email}</a>}
            {contact.phone && <><span className="text-gray-300">•</span><span>{contact.phone}</span></>}
            {contact.website && <><span className="text-gray-300">•</span><a href={contact.website} target="_blank" rel="noopener noreferrer" className="text-blue-700 hover:underline">Website</a></>}
            {contact.socialLinks?.map(link => (
              <React.Fragment key={link.platform}>
                <span className="text-gray-300">•</span>
                <a href={link.url} target="_blank" rel="noopener noreferrer" className="text-blue-700 hover:underline">{link.platform}</a>
              </React.Fragment>
            ))}
        </div>
      </header>
        ...
        <section>
          ...
          {education.map((edu, index) => (
            <div key={index} className="mb-2">
              ...
              {/* CORRECTED: GPA now handles the new object structure */}
         ...
         <section>
          ...
          {projects.map((proj, index) => (
            <div key={index} className="mb-3">
              ...
              {/* CORRECTED: Now maps over the 'links' array using the new LinksList component */}
