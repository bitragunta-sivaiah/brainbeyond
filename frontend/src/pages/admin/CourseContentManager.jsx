...
// Corrected LiveClassItem component
const LiveClassItem = ({ liveClass, handleDeleteLiveClass, handleEditLiveClass }) => {
    // Helper function to format date and time in a readable way
...
const CoursesContentManager = () => {
    ...
    useEffect(() => {
        if (courses.length > 0) {
            const currentSelected = courses.find(c => c._id === selectedCourse?._id);
            setSelectedCourse(currentSelected || courses[0]);
        } else {
    ...
    const handleDeleteLiveClass = useCallback((id, chapterId) => handleDelete(id, "live class", deleteLiveClass, { chapterId }), [handleDelete]);
    ...
    const handleDragEnd = useCallback(
        async (event) => {
            ...
            // Reorder Chapters
            if (itemType === "Chapter") {
                const chapterIds = selectedCourse.chapters.map(c => c._id);
                const oldIndex = chapterIds.indexOf(active.id);
                const newIndex = chapterIds.indexOf(over.id);
                
                try {
                    await dispatch(reorderChapters({ courseId: selectedCourse._id, oldIndex, newIndex })).unwrap();
                } catch (e) {
            ...
            // Reorder Lessons
                ...
                const chapter = selectedCourse.chapters.find(c => c._id === chapterId);
                if (!chapter) return;
                ...
                try {
                    await dispatch(reorderLessons({ chapterId, oldIndex, newIndex })).unwrap();
                } catch (e) {
    ...
    const chapterIds = useMemo(() => selectedCourse?.chapters?.map(c => c._id) || [], [selectedCourse]);
    ...
    return (
        <div className="container mx-auto p-4 md:p-8 bg-background text-foreground min-h-screen">
                    ...
                    <div className="space-y-2">
                        {courses.map(course => (
                            <button
                                ...
                                className={`w-full text-left p-3 rounded-md transition-colors flex items-center gap-3 ${selectedCourse?._id === course._id ? 'bg-primary/10 text-primary font-semibold' : 'hover:bg-muted'}`}
                            >
                        ...
                        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                            <header className="flex justify-between items-start mb-6">
                                <div>
                                    ...
                                    <p className="text-muted-foreground mt-1">{selectedCourse.shortDescription}</p>
                                </div>
                                <div className="flex items-center gap-3 flex-shrink-0">
                                    ...
                                    <button onClick={() => handleEditCourse(selectedCourse)} className="text-green-600 p-2 hover:bg-muted rounded-md" aria-label={`Edit ${selectedCourse.title}`}>
                                        <Edit2 size={18} />
                                    </button>
                                    <button onClick={() => handleDeleteCourse(selectedCourse._id)} className="text-red-500 p-2 hover:bg-muted rounded-md" aria-label={`Delete ${selectedCourse.title}`}>
                                        <Trash2 size={18} />
                                    </button>
                                </div>
                            </header>
            ...
            <AnimatePresence>
                {isChapterFormOpen && (
                    <ChapterForm chapter={editingChapter} courseId={selectedCourse?._id} onClose={() => setIsChapterFormOpen(false)} />
                )}
                {isLessonFormOpen && (
                    <LessonForm 
