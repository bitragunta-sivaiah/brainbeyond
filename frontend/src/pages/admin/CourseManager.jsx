...
const CourseManager = () => {
  ...
  const openModal = (course = null) => {
    if (course) {
      ...
      // Correctly identify if the URL is a placeholder or a real uploaded file
  ...
  return (
    <div className="p-8 bg-background text-foreground min-h-screen">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-heading font-bold">Course Manager</h1>
        <button
          onClick={() => openModal()}
          className="bg-primary text-primary-foreground font-body px-4 py-2 rounded-lg shadow-md hover:bg-primary/90 transition-all duration-300 flex items-center gap-2"
        >
          <Plus size={20} />
          ...
          <ul className="space-y-4">
            {courses.map((course) => (
              <motion.li
                ...
                className="flex items-center justify-between p-4 bg-secondary rounded-lg shadow-sm hover:shadow-md transition-all duration-300"
              >
                <div className="flex items-center space-x-4">
                  <img
                    src={
                      course.thumbnail && !course.thumbnail.startsWith("/path/")
                        ? course.thumbnail
                        : "https://placehold.co/64x64/e9ecef/495057?text=No+Img"
                    }
                    alt={course.title}
                    className="w-16 h-16 object-cover rounded-lg"
                    onError={(e) => {
                      e.target.onerror = null;
                      e.target.src = "https://placehold.co/64x64/e9ecef/495057?text=No+Img";
                    }}
                  />
                  <div>
                    <h3 className="text-lg font-heading font-semibold">{course.title}</h3>
                ...
                <div className="flex space-x-2">
                  ...
                  <motion.button
                    ...
                    className="p-2 bg-destructive/20 text-destructive rounded-full hover:bg-destructive/30 transition-colors"
        ...
        {isModalOpen && (
          ...
          >
            <motion.div
              ...
              className="bg-card p-8 rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto"
            >
              <div className="flex justify-between items-center mb-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                {!isEditMode && (
                    <div className="mb-4 flex items-center gap-2">
                        <label htmlFor="isAiGeneration" className="text-sm font-medium text-muted-foreground font-body">
                ...
                {/* Basic Information */}
                <div>
                    <h3 className="text-xl font-heading font-semibold mb-4 border-b border-border pb-2">
                        Basic Information
                    </h3>
                ...
                {/* Media & Content */}
                <div>
                  <h3 className="mb-4 border-b border-border pb-2 text-xl font-heading font-semibold">
                    Media
                  </h3>
                      ...
                      {thumbnailSource === "url" && (
                        <>
                          <input
                            ...
                            className="w-full rounded-lg border border-border bg-muted px-4 py-2 text-foreground transition-colors focus:border-primary focus:ring-primary"
                          />
                          {formData.thumbnail && (
                            <div className="mt-4">
                              <img
                        ...
                        <div className="mt-4">
                            ...
                            <div className="flex gap-2 mt-1">
                                <input
                                    ...
                                    className="flex-1 px-4 py-2 border rounded-lg border-border bg-muted text-foreground"
                ...
                <div className="flex justify-end gap-4 mt-6">
                  <button
                    type="button"
                    onClick={closeModal}
                    className="px-6 py-2 rounded-lg text-foreground border border-border hover:bg-secondary transition-colors"
                  >
                    Cancel
                  </button>
                  <button
              ...
              {isEditMode && currentCourse && (
                  <div className="mt-8">
                      <h3 className="text-xl font-heading font-semibold mb-4 border-b border-border pb-2">Course Content</h3>
                      {/* Course Content Section */}
                      {currentCourse.chapters && currentCourse.chapters.length > 0 ? (
                          <div className="space-y-4">
                              {currentCourse.chapters.map((chapter) => (
                                  <div key={chapter._id} className="bg-secondary p-4 rounded-lg">
                                      ...
                                      {chapter.lessons && chapter.lessons.length > 0 && (
                                          <ul className="mt-2 space-y-2">
                                              {chapter.lessons.map((lesson) => (
                                                  <li key={lesson._id} className="flex items-center gap-2 p-3 bg-background rounded-md shadow-sm">
                                                      <span className="flex items-center gap-2">
                                                          {lesson.type === 'video' && <VideoIcon size={16} />}
                                                          {lesson.type === 'article' && <FileText size={16} />}
                                                          {lesson.type === 'quiz' && <FileIcon size={16} />}
                          ...
                          <p className="text-center text-muted-foreground font-body">No chapters found for this course.</p>
                      )}
                  </div>
              )}
            </motion.div>
