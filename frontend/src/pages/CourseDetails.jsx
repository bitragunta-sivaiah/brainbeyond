...
const CourseDetails = () => {
    ...
    const { selectedCourse, detailsStatus, actionStatus, paymentStatus, error } = useSelector(
        ...
        if (course.hasAccess) {
            ...
            return (
                <div className="flex flex-col items-center gap-2">
                    ...
                    <span className="text-sm text-custom">Progress: {progressPercent}% Complete</span>
                </div>
            );
        }
    ...
    if (detailsStatus === 'failed' || !course) {
        return (
    ...
    const renderContent = () => (
        <>
            ...
            <section className="bg-card rounded-3xl p-6 md:p-8 shadow mb-8">
                <h2 className="text-2xl font-bold mb-4 font-heading">Course Description</h2>
                <p className="text-custom leading-relaxed whitespace-pre-wrap">
                    {course.description}
                </p>
            </section>
                ...
                {course.hasAccess ? (
                    course.chapters && course.chapters.length > 0 ? (
                        <div className="space-y-4">
                            {course.chapters.map((chapter) => (
                                <div key={chapter._id} className="bg-muted/20 border border-border rounded-xl overflow-hidden">
                                    <button
                                    ...
                                    >
                                        <span className="text-lg font-semibold text-foreground">{chapter.title}</span>
                                        <ChevronDown
                                            className={`w-6 h-6 text-custom transition-transform duration-300 ${isAccordionOpen[chapter._id] ? 'rotate-180' : ''
                                                }`}
                                        />
                                    </button>
                                    <AnimatePresence>
                                        {isAccordionOpen[chapter._id] && (
                                            ...
                                            >
                                                <ul className="p-4 space-y-2">
                                                    {course.chapters
                                                        .find(c => c._id === chapter._id)?.lessons.map((lesson) => (
                                                            <li key={lesson._id} className="flex items-center gap-3 text-custom">
                        ...
                        <p className="text-custom italic">No curriculum has been added to this course yet.</p>
                    )
                ) : (
                    <div className="relative w-full h-96 flex items-center justify-center">
                        ...
                        <div className="relative z-20 text-center flex flex-col items-center">
                            ...
                            <h3 className="text-3xl font-bold text-foreground font-heading">Course Content Locked</h3>
                            <p className="text-custom mt-2 max-w-sm">
                                To unlock the full curriculum and all lessons, please purchase or enroll in this course.
                            </p>
                        </div>
                    </div>
                )}
            ...
            <section className="bg-card rounded-3xl p-6 md:p-8 shadow">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-1">
                                        {[...Array(5)].map((_, i) => (
                                            <Star
                                                key={i}
                                                className={`cursor-pointer w-6 h-6 transition-colors ${i < reviewData.rating ? 'text-yellow-400' : 'text-gray-300'
                                                    }`}
                ...
                {course.reviews && course.reviews.length > 0 ? (
                                ...
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2">
                                            <span className="font-semibold text-foreground">{review.user.fullName}</span>
                                            <MdVerified className="text-primary text-base" />
                                        </div>
                                        <div className="flex items-center gap-1">
                                            <Star className="text-yellow-400 w-4 h-4" fill="#facc15" />
                                            <span className="font-semibold text-foreground text-sm">{review.rating.toFixed(1)}</span>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <p className="text-custom italic">This course does not have any reviews yet.</p>
                )}
            </section>
        </>
    );
        ...
        <div className="w-full max-w-7xl mx-auto px-3 py-8 sm:px-6 lg:px-8 font-body">
            ...
            >
                ...
                <div className="relative z-10 h-full flex flex-col justify-end p-6 md:p-10 text-primary-foreground">
                    ...
                    <p className="text-lg mb-6 leading-relaxed max-w-2xl">{course.shortDescription}</p>
                    <div className="flex items-center gap-6 text-sm flex-wrap">
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            <span className="text-xs">({course.totalRatings} ratings)</span>
                        </div>
                    ...
                    <div className="sticky top-8 space-y-6">
                        <div className="bg-card rounded-3xl shadow p-6 flex flex-col">
                            ...
                            <div className="relative w-full aspect-video rounded-xl overflow-hidden shadow-md mb-6">
                                ...
                                >
                                    <Play className="w-12 h-12 text-primary-foreground" fill="#fff" />
                                </button>
                            </div>
                            <div className="w-full flex flex-col gap-4">
                                <div className="flex items-end justify-center gap-2">
                                    <span className="text-4xl font-bold text-foreground font-heading">
                                        {course.isFree ? 'Free' : `₹${(course.discountedPrice || course.price).toFixed(2)}`}
                                    </span>
                                    {course.discountedPrice && (
                                        <span className="line-through text-custom text-lg">₹{course.price.toFixed(2)}</span>
                                    )}
                                </div>
                                {renderEnrollmentButton()}
                                <div className="text-center text-sm text-custom">30-day money-back guarantee</div>
                            </div>
